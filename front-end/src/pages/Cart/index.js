...
class Cart extends PureComponent{
    ...
    changeAmount=(id)=>{
        ...
        //修改选中状态
        axios.get(`http://localhost:8085/cart/changeSelectStatus?cartId=`+id,{
            headers:{
    ...
    getData=()=>{
        //查找购物车总体数据
        axios.get(`http://localhost:8085/cart/selectCart`,{
            headers:{
                'token':localStorage.getItem('user_token')
            }
        }).then(
        ...
        //查找总价格
        axios.get(`http://localhost:8085/cart/totalPrice`,{
            headers:{
                'token':localStorage.getItem('user_token')
            }
        }).then(
        ...
        axios.get(`http://localhost:8085/cart/chosen`,{
            headers:{
                'token':localStorage.getItem('user_token')
            }}).then(
            res=>{
    ...
    deleteCart =(id)=>{
        axios.delete(`http://localhost:8085/cart/delete?productId=`+id,{
            headers:{
                'token':localStorage.getItem('user_token')
            }
        }).then(
    ...
    changeProductCount=(event,cartId)=>{
        ...
        axios.get(`http://localhost:8085/cart/changeCount?cartId=`+cartId+`&quantity=`+quantity,{
            headers:{
                'token':localStorage.getItem('user_token')
            }
        }).then(
