...
class RestaurantDetail extends  PureComponent{
    ...
    getData=(cafeId)=>{
        axios.get(`http://localhost:8085/restaurant/`+cafeId).then(
            res=>{
                if(res.data.err!==0){
        ...
        axios.get(`http://localhost:8085/restaurant/comment/`+cafeId).then(
            res=>{
                if(res.data.err!==0){
    ...
    publishComment = (id,commentText,score)=>{
        axios.post('http://localhost:8085/addCafeComment',
            {
                headers:{
